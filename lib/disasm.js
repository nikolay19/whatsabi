...
function isUint8ArrayASCII(uint8Array) {
...
function disasm(bytecode) {
    ...
    while (code.hasMore()) {
        ...
        if (inst === opcodes_1.opcodes.PUSH32) {
            console.log("!!OK!!CHECK ASCII"+ethers_1.ethers.utils.hexlify(code.value()));
            if(isUint8ArrayASCII(cutEvent(code.value()))) {
                console.log("!!OK!!SKIP ASCII "+ethers_1.ethers.utils.hexlify(code.value()));
                continue
            }
            ...
            console.log("!!OK!!PUSH EVENT "+ethers_1.ethers.utils.hexlify(lastPush32));
