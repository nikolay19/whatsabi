...
function valueToOffset(value) {
    ...
    return parseInt((0, bytes_1.hexlify)(value), 16);
}
...
class BytecodeIter {
    ...
    constructor(bytecode, config) {
        ...
        this.posBuffer = [];
        this.bytecode = (0, bytes_1.arrayify)(bytecode, { allowMissingPrefix: true });
    }
    hasMore() {
...
function abiFromBytecode(bytecode) {
    //console.log("abiFromBTYECODE IN LIB 1");
    const p = disasm(bytecode);
    //console.log("disasm selectors")
    //console.log(p.selectors);
    const abi = [];
    for (const [selector, offset] of Object.entries(p.selectors)) {
        if (selector == "0x00000001"||selector == "0x00000002") {
            continue
        }
        ...
        if (!(offset in p.dests)) {
            //console.log(selector,"// Selector does not point to a valid jumpdest. This should not happen.")
            continue;
        }
...
function disasm(bytecode) {
    ...
    while (code.hasMore()) {
        ...
        //console.log("process "+code.at(pos));
        ...
        if (inst === opcodes_1.opcodes.PUSH32) {
            ...
            p.eventCandidates.push((0, bytes_1.hexlify)(lastPush32));
            continue;
        }
        // Find JUMPDEST labels
        if (inst === opcodes_1.opcodes.JUMPDEST) {
            // End of the function, or disjoint function?
            //console.log("JUMPDEST, check isHalt", code.at(-2));
            if ((0, opcodes_1.isHalt)(code.at(-2)) || code.at(-2) === opcodes_1.opcodes.JUMP) {
                ...
                if (checkJumpTable && Object.keys(p.selectors).length > 0) {
                    //console.log(p);
                    //console.log("checkJumpTable = false");
                ...
                if (resumeJumpTable.delete(pos)) {
                    ...
                    //if(!checkJumpTable)console.log("checkJumpTable ", checkJumpTable, code.at(pos+1).toString(16));
                }
            ...
            //console.log("// TODO: Check whether function has a simple return flow?")
            continue;
        }
        // Annotate current function
        if (currentFunction.opTags !== undefined) {
            //console.log("// Detect simple JUMP/JUMPI helper subroutines");
            if ((inst === opcodes_1.opcodes.JUMP 
                || inst === opcodes_1.opcodes.JUMPI) && (0, opcodes_1.isPush)(code.at(-2))) {
        ...
        if (!checkJumpTable) {
            //console.log("// Skip searching for function selectors at this point")
        ...
        if (!(code.at(-1) === opcodes_1.opcodes.JUMPI && (0, opcodes_1.isPush)(code.at(-2)))){
            //console.log("// Beyond this, we're only looking with instruction sequences that end with ")
            continue;
        }
        const offsetDest = valueToOffset(code.valueAt(-2));
        ...
        if (code.at(-3) === opcodes_1.opcodes.EQ &&
            ...
            if (value.length < 4) {
                ...
                value = (0, bytes_1.zeroPad)(value, 4);
            }
            const selector = (0, bytes_1.hexlify)(value);
